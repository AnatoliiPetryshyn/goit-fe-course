...
const getAllNames = users => users.map(item => item.name);
...
const getUsersByGender = (users, gender) => {
  const filteredBygender = users.filter(item => item.gender === gender);
  const getNames = filteredBygender.map(item => item.name);
  return getNames;
};
...
const getTotalBalance = users =>
  users.reduce((total, item) => {
    const totalBalance = total + item.balance;
    return totalBalance;
  }, 0);
